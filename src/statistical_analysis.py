...
def hypothesis_test_by_strategy(data: pd.DataFrame, expected_return: float, significance_level: float = 0.05):
    ...
    Args:
        data (pd.DataFrame): DataFrame contendo as colunas ['symbol', 'model', 'return'].
    ...
    for (symbol, strategy), group in data.groupby(["symbol", "model"]):
        returns = group["return"].dropna()

        # Realiza o teste t de uma amostra
        t_stat, p_value = ttest_1samp(returns, expected_return / 100)

        # Conclusão do teste
        conclusion = "Rejeitar H0" if p_value < significance_level else "Não rejeitar H0"
...
if __name__ == "__main__":
    ...
    for (symbol, strategy), result in results_by_strategy.items():
        ...
        group_data = data_filtered[(data_filtered['symbol'] == symbol) & (data_filtered['model'] == strategy)]
        achieved_mean = group_data['return'].mean() * 100  # Converte para porcentagem
