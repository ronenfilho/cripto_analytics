...
def parse_args():
    parser = argparse.ArgumentParser(description="Pipeline de previsão de preços de criptomoedas")
    
    parser.add_argument("--data", action="store_true", help="(1) Executa a carga e transformação do dados")
    parser.add_argument("--features", action="store_true", help="(2) Executa o cálculo de features")
    parser.add_argument("--model", action="store_true", help="(3) Executa o treinamento de modelos")    
    parser.add_argument("--simulate", action="store_true", help="(4) Executa a simulação de investimento")
    #TODO
    ...
    parser.add_argument("--crypto", type=str, help="Símbolos das criptomoedas a serem analisadas. Exemplo: BTC/USDT,ETH/USDT. Todos os símbolos disponíveis na pasta raw serão utilizados se não for especificado. Permitidos: BCH/USDT,BTC/USDT,DASH/USDT,EOS/USDT,ETC/USDT,ETH/USDT,LTC/USDT,XMR/USDT,XRP/USDT,ZRX/USDT")    
    parser.add_argument("--kfolds", type=int, default=5, help="Número de folds para validação cruzada (K-Fold)")
...
if __name__ == "__main__":
    ...
    try:
        ...
        if args.crypto or args.model or args.days or args.capital:
            logging.info("Atualizando variáveis de ambiente com base nos argumentos fornecidos.")
        ...
        if not any([args.data, args.features, args.model, args.simulate]):
            logging.info("Nenhum argumento fornecido do pipeline. Processando todas as etapas do pipeline...")
            data_load_main()
            features_main()
            models, data_calculate = models_main()
            simulate_main(data_calculate, models=models)
