...
def calculate_summary_statistics(data: pd.DataFrame, output_filename: str = "summary_statistics.csv", symbols: list = None) -> pd.DataFrame:
    ...
    for symbol, group in data.groupby("symbol"):
        ...
        mode_value = round(close_prices.mode().iloc[0], 2) if not close_prices.mode().empty else None
...
def calculate_dispersion_measures(data: pd.DataFrame, output_filename: str = "dispersion_statistics.csv", symbols: list = None) -> None:
...
def analisar_medidas_dispersao(data: pd.DataFrame, output_filename: str = "analise_dispersao.csv", symbols: list = None) -> pd.DataFrame:
    ...
    """
    logger.info("Iniciando análise comparativa detalhada das medidas de dispersão entre criptomoedas")
    logger.debug(f"Tamanho do DataFrame recebido: {data.shape}")
    ...
    logger.info("Classificação das criptomoedas por volatilidade (coeficiente de variação):")
    for i, row in df_results.iterrows():
        logger.info(f"{i+1}. {row['symbol']}: CV = {row['coef_variacao']}%, Desvio Padrão = {row['desvio_padrao']}")
    
    return df_results

def visualizar_comparacao_dispersao(df_dispersao: pd.DataFrame, dados_originais: pd.DataFrame) -> None:
    ...
    metrics = ['desvio_padrao', 'variancia', 'coef_variacao', 'amplitude_interquartil', 'desvio_medio_absoluto']
    ...
    for i, (_, row) in enumerate(df_radar.iterrows()):
        ...
        ax.plot(angles, values, 'o-', linewidth=2, color=colors[i], label=row['symbol'], alpha=0.8)
...
def main() -> None:
    ...
    df_dispersao = analisar_medidas_dispersao(filtered_data, "analise_dispersao.csv", symbols)
